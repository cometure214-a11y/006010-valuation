...
# --- P0 优化参数（2026-08-26 多 Agent 分析结论落地）---
# P0-1 篮子 MAD 截尾：剔除偏离中位数超过 TRIM_MAD×MAD 的极端个股（≈3σ 截尾）。
# 依据：08-20 复盘——太辰光单日 +18.38% 把篮子均值拉高约 1pp，MAD 截尾可显著缓解。
TRIM_MAD = 4.5                # 4.5×原始MAD ≈ 3×1.4826×MAD ≈ 3σ
# P0-2 P5 最小权重保护：P5 全段 MAE 0.718% 高于行业模型，会被择优闸门(gate)误淘汰，
# 但 P5 是独立信息源（个股反推），在极端分化日误差反而最小（08-20 仅 +0.40pp）。
# 给最小权重下限，避免"全段平均劣势"掩盖"分化日优势"。
# 调优：原 0.10 太激进导致 45%，回归 0.15-0.20 区间。
P5_MIN_WEIGHT = 0.15
# P5 动态剔除（fix #1，2026-08-28）：P5 个股反推在暴涨/暴跌日常失真
# （如 08-27 暴涨日 P5 喊 -0.76% 而实盘 +6.96%），此时占位 MAE 毫无意义、
# 0.15 下限反而拖累集成。判定 P5 偏离其他模型共识 > P5_OUTLIER_SIGMAS·σ 时，
# 视为脏信号、当日剔除（权重归 P1~P4）；正常日不触发，P0-2 下限照常生效。
P5_OUTLIER_SIGMAS = 2.0
# 绝对偏差下限(pp)：仅在 |P5-共识| 同时超过「相对 2σ」与「该绝对下限」时才剔除。
# 必要性：平静日 P1~P4 共识极紧(σ≈0.03)，P5 正常 ±0.1pp 摆动也会触发 2σ → 会几乎每天误杀 P5、
# 架空 P0-2；波动日他人本身离散(σ≈0.7)，P5 偏 1.6pp 属合理分歧，也不应剔除。
# 本修复针对的是「暴涨/暴跌日 P5 喊反方向、误差达数 pp」的灾难性失真（如 08-27 P5=-0.76% vs
# 共识 +6.08%，偏差 6.8pp）。取 2.0pp：仅捕获这类灾难性误差，正常/波动日 P5 的 15% 权重照常保留。
...
def ensemble_weights(maes, groups=None, cap=MODEL_WEIGHT_CAP, floor=MAE_FLOOR,
                     ...
                     provisional_cap=PROVISIONAL_WEIGHT_CAP, min_weights=None):
    """
    ...
    orig_maes = dict(maes)
    dropped = []
    # P0-2：min_weights 指定的模型（如 P5 独立信息源）豁免淘汰闸门——
    # 其"全段平均 MAE 劣势"不应掩盖"分化日优势"，不能因 gate 被直接剔除。
    _gate_exempt = set(min_weights or ())
    if gate and gate > 0 and len(maes) > 1:
        best_mae = min(maes.values())
        thr = (max(best_mae, 0.0) + floor) * gate
        kept = {m: v for m, v in maes.items()
                if (max(v, 0.0) + floor) <= thr or m in _gate_exempt}
    ...
    # 5.5) 最小权重保护（P0-2）：对指定模型强制提升到下限，再重新归一化。
    #     只对幸存模型生效（被闸门淘汰者不复活），迭代至所有下限同时满足。
    if min_weights:
        for _ in range(12):
            w = {m: max(v, min_weights.get(m, 0.0)) for m, v in w.items()}
            tot = sum(w.values())
            if tot <= 0:
                break
            w = {m: v / tot for m, v in w.items()}
            if all(w.get(m, 0.0) >= min_weights.get(m, 0.0) - 1e-9
...
def p5_should_exclude(models, p5_key="P5_个股辅助", sigmas=P5_OUTLIER_SIGMAS,
    ...
    暴涨/暴跌日常失真），返回 (True, diag) 供调用方当日剔除 P5、权重归一给 P1~P4：
      ...
      2) |P5 - mu| > abs_floor   （绝对偏差达 ~1pp，排除平静日噪声误杀）
    其中 mu=其他模型均值，σ=其他模型预测的样本标准差。

    正常日 P5 贴近共识、绝对偏差 < 下限 → 不触发，P0-2 的 0.15 下限照常生效。
...
def bias_correction(errs, hl=10, max_abs=1.5, div_tol=0.35,
    ...
    """
    历史偏差修正（方法文档 §六）+ 防滞后（本轮新增）+ 动量增强（P0-3）。
    ...
    # P0-3 动量增强：近端误差持续同向 → 放大修正（连续同向是系统性偏差信号，非噪声）
    momentum = 0.0
    recent = arr[-momentum_window:]
    if len(recent) >= 3:
        pos = int(np.sum(recent > 0))
        neg = int(np.sum(recent < 0))
        same_ratio = max(pos, neg) / len(recent)
        if same_ratio >= momentum_ratio:
            momentum = momentum_boost * (same_ratio - momentum_ratio) / (1.0 - momentum_ratio)
    applied *= (1.0 + momentum)
